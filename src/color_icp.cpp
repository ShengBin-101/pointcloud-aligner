...
class ColorICP {
  ...
  private:
    ...
    bool estimate_normal_ = true;
    double voxel_resolution_ = 0.01;  // 1 cm
    double normal_est_radius_ = 0.05; // 5 cm
    double search_radius_ = 0.04;     // 4 cm
    std::string icp_method_ = "color"; // pcl, classic, color
    double icp_max_corres_dist_ = 0.05; // 5 cm
    double icp_transformation_epsilon_ = 0.000001; // 0.001 meter and 0.256 degree
    int icp_max_iterations_ = 100;
    double color_icp_lambda_ = 0.968; // default 0.968 as in the paper
    std::string color_weight_ = "average"; // average, srgb, ntsc
    bool color_visualization_ = true; // visualize the registration with color
};
...
void ColorICP::setSourceCloud(const PointCloudPtr& source) {
    *source_cloud_ = *source;
    
    // Pre-process the source cloud
    removeNaNPoints(source_cloud_);
    if (downsample_) downSampleVoxelGrids(source_cloud_);
    if (estimate_normal_) estimateNormals(source_cloud_);
}

void ColorICP::setTargetCloud(const PointCloudPtr& target) {
    *target_cloud_ = *target;
    
    // Pre-process the target cloud
    removeNaNPoints(target_cloud_);
    if (downsample_) downSampleVoxelGrids(target_cloud_);
    if (estimate_normal_) estimateNormals(target_cloud_);
    
    // Prepare color gradient for the target
    prepareColorGradient(target_cloud_);
}

void ColorICP::setParameters(bool downsample, bool estimate_normal,
                          double voxel_resolution, double normal_est_radius,
                          double search_radius, double color_icp_lambda,
                          double icp_max_corres_dist) {
    downsample_ = downsample;
    estimate_normal_ = estimate_normal;
    voxel_resolution_ = voxel_resolution;
    normal_est_radius_ = normal_est_radius;
    search_radius_ = search_radius;
    color_icp_lambda_ = color_icp_lambda;
    icp_max_corres_dist_ = icp_max_corres_dist;
}

Eigen::Matrix4d ColorICP::perform(const PointCloudPtr& source, const PointCloudPtr& target) {
...
void ColorICP::downSampleVoxelGrids (const PointCloudPtr& cloud) {
  double resolution = voxel_resolution_;
...
void ColorICP::estimateNormals (const PointCloudPtr& cloud) {
  ...
  ne.setRadiusSearch (normal_est_radius_);
...
Eigen::Matrix4d ColorICP::PCLICP (const PointCloudPtr& source, const PointCloudPtr& target) {
  ...
  icp.setMaxCorrespondenceDistance (icp_max_corres_dist_);
...
Eigen::Matrix4d ColorICP::ClassicICPRegistration (const PointCloudPtr& source, const PointCloudPtr& target) {
  // initialization
  int iteration = icp_max_iterations_;
  double translation_epsilon = icp_transformation_epsilon_;
  double rotation_epsilon = 1.0 - icp_transformation_epsilon_;
  double distance_threshold = icp_max_corres_dist_; // icp.setMaxCorrespondenceDistance
...
void ColorICP::prepareColorGradient (const PointCloudPtr& target) {
  ...
  double search_radius = search_radius_; // search radius for color gradient estimation
...
Eigen::Matrix4d ColorICP::GaussNewtonWithColor (const PointCloudPtr& source, const PointCloudPtr& target, 
  ...
  double lambda = color_icp_lambda_; // TODO: test varying param values
