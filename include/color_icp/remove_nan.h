                                       ...
                                       std::vector<int> &index)
{
  ...
  else
  {
    ...
    {
      if (!isfinite(cloud_in.points[i].histogram[0]))  // PCL v1.8
        continue;
                                       ...
                                       std::vector<int> &index)
{
  ...
  else
  {
    for (size_t i = 0; i < cloud_in.points.size (); ++i)
    {
      if (!isfinite (cloud_in.points[i].descriptor[0]) ||    // PCL v1.8
          !isfinite (cloud_in.points[i].rf[0]))              // PCL v1.8
        continue;
      cloud_out.points[j] = cloud_in.points[i];
      index[j] = static_cast<int>(i);
      j++;
                                             ...
                                             std::vector<int> &index)
{
  ...
  for (size_t i = 0; i < cloud_in.points.size (); ++i)
  {
    if (!isfinite (cloud_in.points[i].x) ||
        !isfinite (cloud_in.points[i].y) ||
        !isfinite (cloud_in.points[i].z))
      continue;
    cloud_out.points[j] = cloud_in.points[i];
    index[j] = static_cast<int>(i);
    j++;
